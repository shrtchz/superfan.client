...
// Input Verification Modal Component - Exact match to screenshot
const InputVerificationModal: React.FC<{
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="p-0 gap-0 max-w-[400px] rounded-2xl border border-gray-200 bg-white dark:bg-black overflow-hidden" showCloseButton={false}>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <span className="text- font-bold text-black dark:text-white">Your Answer</span>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="px6 pb-2 pt4">
      
          <div className=" p-3 rounded border-b border-gray-200 ">
            <p className="font-semibold text-black dark:text-white text-">{userInput}</p>
          </div>
        </div>
{/* dddd */}
        {/* Instructions */}
        <div className="px-2 pb- border-b">
          <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed ">
            From the options below, pick the one most similar to your response and check it to continue.
          </p>
        </div>

        {/* Options - Updated with your quiz styling */}
            ...
            return (
              ...
              >
                ...
                {isSelected && !showFeedback && (
                  <Check size={20} className="absolute right-4 text-white dark:text-black" />
                )}
                ...
                {showFeedback && isSelected && (
                  <>
                    {isAnswerCorrect ? (
                      <Check size={20} className="absolute right-4 text-white" />
                    ) : (
                      <X size={20} className="absolute right-4 text-white" />
                    )}
                  </>
                )}
              </button>
        ...
        <div className="px-6 py-4">
          <button
            ...
            className="w-full py-2 bg-black dark:bg-white text-white dark:text-black font-bold rounded-full hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
          >
            Check answer
          </button>
        </div>
