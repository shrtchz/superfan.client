...
import SubjectIcon from "@/public/icons/SubjectIcon";
import ArrorwIcon from "@/public/icons/ArrowIcon";

export default function MessageInput() {
	const [message, setMessage] = useState("");
	const [selectedPlan, setSelectedPlan] = useState<"1month" | "1year">(
		"1month"
	); // Default to 1month
	...
	return (
		<div className="flex flex-col justify-center gap-4 dark:text-white text-black itemscenter w-full flex-1 h-full border border-t-0 border-b-0  ">
			<div
				className={`w-full justifycenter flex items-center gap-4 flex-col h-[30%]  relative `}
			>
				...
				<div className="border border-gray-400/30 rounded-2xl h-28 w-2/3 2xl:w-1/3">
					<form
						onSubmit={handleSubmit}
						className="relative w-full h-[70%] border-b p-1"
					>
						<Input
							type="text"
							placeholder="Request me to create a quiz"
							value={message}
							onChange={(e) => setMessage(e.target.value)}
							className="w-full  border-0 focus-visible:border-0 focus-visible:ring-0 shadow-none outline-none  textcenter text-base rounded-none placeholder:text-base"
					...
					<div className="w-full flex justify-between h-max items-center px-2">
						<DropdownMenu>
