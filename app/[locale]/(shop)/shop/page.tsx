...
function ProductModal({ product, open, onOpenChange }: { product: Product | null; open: boolean; onOpenChange: (v: boolean) => void; }) {
  ...
  return (
		...
		>
			<DialogContent className="sm:max-w-3xl  w-full h-[500px] gap-0 p-0 overflow-hidden border-white/20">
				<div className="flex h-full ">
							...
							{product.images.map((src, i) => (
								<button
									...
									className={`rounded-md p-1 border ${
										selectedImage === i ? "ring-2 ring-blue-400" : "opacity-60"
									}`}
								>
									<img
										src={src}
					...
					{/* Large Image */}
					<div className="flex-1 bg-[#e9f0f2] p-4 flex flex-col gap-4 items-center justify-center dark:text-black">
						<img
							src={product.images[selectedImage]}
							alt={product.title}
							className="max-h-full max-w-full object-contain"
						/>
						<div className="flex items-center border dark:border-gray-300 rounded-lg overflow-hidden">
							<button
								onClick={() => setQty((q) => Math.max(1, q - 1))}
								className="px-3 py-2 hover:bg-gray-100 dark:hover:text-black transition-colors"
					...
					<div className="flex-1 p-4 space-y-4 overflow-y-auto">
								...
								{product.colors.map((colorName) => (
									<button
										...
										className={`w-5 h-5 border rounded-full flex items-center justify-center ${
											selectedColor === colorName ? "ring-2 ring-black" : ""
										}`}
										style={{ backgroundColor: colorName || "#cccccc" }}
										title={colorName}
						...
						{/* Sizes */}
						<div>
							<div className="text-sm text-gray-600 dark:text-white">Select size</div>
						...
						<div className="flex items-center w-full gap-4">
							<Button
								...
								className="flex-1 py-3 rounded-full font-semibold transition border border-white/20 dark:bg-inherit dark:hover:bg-white dark:hover:text-black dark:text-white"
								variant="outline"
							>
								Add to Cart
							</Button>
						...
						<div>
							...
							<ul className="mt-2 text-sm text-gray-600 dark:text-white list-disc list-inside space-y-1">
